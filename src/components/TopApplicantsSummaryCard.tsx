...
import { fetchTopApplicantsSummary, ApplicantSummary } from "../services/insights.api";

export const TopApplicantsSummaryCard = ({ dateRange }: { dateRange?: { start: string, end: string } }) => {
  ...
  const loadData = async (p: number, append = false) => {
    setLoading(true);
    try {
      const data = await fetchTopApplicantsSummary(p, 10, dateRange);
      if (append) {
  ...
  const downloadCSV = async () => {
    const allData = await fetchTopApplicantsSummary(1, 'all', dateRange);
    ...
    a.href = url;
    a.download = "top_applicants_summary.csv";
    a.click();
    window.URL.revokeObjectURL(url);
  };

  const List = ({ items }: { items: ApplicantSummary[] }) => (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
      <thead>
        <tr style={{ textAlign: "left", color: "#666", borderBottom: "1px solid #eee" }}>
          ...
          <th style={{ padding: "8px 0" }}>Most Applied Role</th>
        </tr>
      </thead>
      <tbody>
        {items.map((item, i) => (
          <tr key={`${item.userId}-${i}`} style={{ borderBottom: "1px solid #f5f5f5" }}>
            ...
            <td style={{ padding: "12px 0", textAlign: "center", fontWeight: 600 }}>{item.totalApplications}</td>
            <td style={{ padding: "12px 0", color: "#666" }}>{item.mostAppliedRole}</td>
          </tr>
        ))}
      </tbody>
    </table>
    ...
    <>
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ margin: 0, fontSize: 16 }}>Top Applicants (Summary)</h3>
      ...
      {showModal && (
        <div style={{
          ...
          <div style={{
            background: "white", width: "700px", maxHeight: "80vh", borderRadius: 12, padding: 24,
