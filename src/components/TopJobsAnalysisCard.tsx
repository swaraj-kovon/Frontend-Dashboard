...
import { fetchTopJobsAnalysis, TopJobsAnalysisItem } from "../services/insights.api";

export const TopJobsAnalysisCard = ({ dateRange }: { dateRange?: { start: string, end: string } }) => {
  const [items, setItems] = useState<TopJobsAnalysisItem[]>([]);
  ...
  const loadData = async (p: number, append = false) => {
    setLoading(true);
    try {
      const data = await fetchTopJobsAnalysis(p, 10, dateRange);
      if (append) {
  ...
  const downloadCSV = async () => {
    const allData = await fetchTopJobsAnalysis(1, 'all', dateRange);
    ...
    a.href = url;
    a.download = "top_jobs_analysis.csv";
    a.click();
    window.URL.revokeObjectURL(url);
  };

  const List = ({ data }: { data: TopJobsAnalysisItem[] }) => (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
      <thead>
        <tr style={{ textAlign: "left", color: "#666", borderBottom: "1px solid #eee" }}>
          <th style={{ padding: "8px 0" }}>Job Role</th>
          <th style={{ padding: "8px 0", textAlign: "right" }}>Job Posts</th>
          <th style={{ padding: "8px 0", textAlign: "right" }}>Interested Users</th>
        </tr>
      </thead>
      <tbody>
        {data.map((item, i) => (
          <tr key={`${item.role}-${i}`} style={{ borderBottom: "1px solid #f5f5f5" }}>
            ...
            <td style={{ padding: "12px 0", textAlign: "right" }}>{item.userCount}</td>
          </tr>
        ))}
      </tbody>
    </table>
    ...
    <>
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ margin: 0, fontSize: 16 }}>Top Jobs Analysis (Supply vs Demand)</h3>
          ...
          <div style={{ background: "white", width: "700px", maxHeight: "80vh", borderRadius: 12, padding: 24, display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <h2 style={{ margin: 0, fontSize: 20 }}>Top Jobs Analysis</h2>
