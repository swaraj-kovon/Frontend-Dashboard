...
import { fetchIncompleteProfiles, IncompleteProfile } from "../services/insights.api";

export const IncompleteProfilesCard = ({ dateRange }: { dateRange?: { start: string, end: string } }) => {
  ...
  const loadData = async (p: number, append = false) => {
    setLoading(true);
    try {
      const response = await fetchIncompleteProfiles(p, 10, dateRange);
      if (append) {
  ...
  const downloadCSV = async () => {
    const response = await fetchIncompleteProfiles(1, 'all', dateRange);
    ...
    a.href = url;
    a.download = "incomplete_profiles.csv";
    a.click();
    window.URL.revokeObjectURL(url);
  };

  const List = ({ items }: { items: IncompleteProfile[] }) => (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
      <thead>
        <tr style={{ textAlign: "left", color: "#666", borderBottom: "1px solid #eee" }}>
          <th style={{ padding: "8px 0" }}>User ID</th>
          <th style={{ padding: "8px 0" }}>Missing Fields</th>
        </tr>
      </thead>
      <tbody>
        {items.map((item, i) => (
          <tr key={`${item._id}-${i}`} style={{ borderBottom: "1px solid #f5f5f5" }}>
            <td style={{ padding: "12px 0", color: "#999", fontSize: 12, fontFamily: "monospace" }}>
              {item._id}
            </td>
            <td style={{ padding: "12px 0", color: "#d93025" }}>
              {[
                !item.fullName && "fullName",
                !item.targetCountry && "targetCountry",
                !item.targetJobRole && "targetJobRole"
              ].filter(Boolean).join(", ")}
            </td>
          </tr>
        ))}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ margin: 0, fontSize: 16 }}>
            Incomplete Profiles <span style={{ color: "#999", fontWeight: "normal", marginLeft: 8 }}>({total})</span>
          </h3>
          <button onClick={handleShowMore} style={{ background: "none", border: "none", color: "#007bff", cursor: "pointer", fontSize: 14 }}>
            Show More
          </button>
        </div>
        <List items={users.slice(0, 6)} />
      </div>

      {showModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000 }}>
          <div style={{ background: "white", width: "600px", maxHeight: "80vh", borderRadius: 12, padding: 24, display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <h2 style={{ margin: 0, fontSize: 20 }}>Users with Incomplete Details</h2>
