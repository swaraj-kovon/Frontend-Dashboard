...
import { fetchJobStatusList, JobStatusItem } from "../services/insights.api";

export const JobStatusListCard = ({ dateRange }: { dateRange?: { start: string, end: string } }) => {
  const [items, setItems] = useState<JobStatusItem[]>([]);
  ...
  const loadData = async (p: number, append = false) => {
    setLoading(true);
    try {
      const data = await fetchJobStatusList(p, 10, dateRange);
      if (append) {
  ...
  const downloadCSV = async () => {
    const allData = await fetchJobStatusList(1, 'all', dateRange);
    ...
    a.href = url;
    a.download = "job_status_list.csv";
    a.click();
    window.URL.revokeObjectURL(url);
  };

  const List = ({ data }: { data: JobStatusItem[] }) => (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
      ...
      <tbody>
        {data.map((item, i) => (
          <tr key={`${item.companyName}-${item.title}-${i}`} style={{ borderBottom: "1px solid #f5f5f5" }}>
            <td style={{ padding: "12px 0", fontWeight: 500 }}>{item.companyName}</td>
            <td style={{ padding: "12px 0", color: "#333" }}>{item.title}</td>
            <td style={{ padding: "12px 0", textAlign: "right" }}>
              <span style={{
                padding: "4px 8px", borderRadius: 4, background: "#f0f0f0", fontSize: 12, fontWeight: 600
              }}>
    ...
    <>
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ margin: 0, fontSize: 16 }}>Job Status List</h3>
          ...
          <div style={{ background: "white", width: "900px", maxHeight: "80vh", borderRadius: 12, padding: 24, display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <h2 style={{ margin: 0, fontSize: 20 }}>All Jobs Status</h2>
